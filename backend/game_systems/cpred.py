...
- backdoor_entry: {type, character, target (destination node), interface_rank?} — Project rulebook §Net Actions: enter a node stealthily as the post-stealth-break fallback. Backend rolls Interface + 1d10 vs HIGHEST active ICE PER in destination + 1d10 (no Cloak booster — RAW specifies Interface only). Costs 1 NA. Always emits `node_change` (you enter regardless). Pass: emits `backdoor_entry_grace` op — all ICE in the destination skip encounter rolls THIS TURN (Speed Check + Patrol detection auto fail-soft). Fail: ICE in destination triggers as if a normal Move Node — planner emits the Speed Check / Patrol detection on the same turn. Use when stealth_active=False; while stealthed, use `stealth_contest` instead (better odds via Cloak, silent bypass on success).
...
- backdoor_entry: {type, character, target (destination node), interface_rank?} — Project rulebook stealth-move action. 1 NA. Interface + d10 vs HIGHEST active ICE PER in destination + d10 (no Cloak booster). Always enters node. Pass: emits `backdoor_entry_grace` op so ICE in destination skip encounter rolls this turn. Fail: ICE triggers normally — planner emits Speed Check / Patrol detection same turn.
