...
**Do not truncate, compress, or rush the narration because it lives inside a JSON field.** Let scenes breathe at their natural length. If a beat wants two paragraphs, write two; if it wants seven, write seven. Dialogue gets its full back-and-forth. NPCs get their layered reactions, mannerisms, accent slips, the small physical tells that make them dimensional. Atmosphere gets sensory grounding (light, sound, scent, texture, distance). Night City's grit, Kabuki's paper-lantern hum, Watson's working-class warmth, the noir-with-warmth tone — none of it gets shortcut. The narration string is effectively unbounded for the scene's needs. A scene that reads thin because the model filled the field too quickly is a failure even if the state is correct.
...
- **`quiet_jack_in`** {type, character, interface_rank?}: establishes stealth on jack-in. **Costs 1 NA** (RAW: "spend an additional NET Action when you Jack In" — that's the price of the stealthy entrance, no offset). Resolves contested Interface + 1d10 vs every Watcher's Interface + Pathfinder + 1d10. Pass requires beating ALL Watchers. Cloak booster bonuses (Eraser +2) fire automatically on the contest itself. Once per encounter (re-Jack-In requires Jack Out first). NA is consumed even on failure.
  ...
  - **HOMERULE — Eraser auto-activates:** If Eraser is loaded in the runner's deck_slots and currently `status="deactivated"`, the resolver auto-activates it as part of the Quiet Jack-In (sets Eraser status to active, emits a `program_status_change` op). Fires on EITHER pass or fail — the NA was spent either way, and going to the trouble of jacking in quietly without your Cloak booster running is mechanically incoherent (Patrol ICE can be at Gateway, the runner needs Cloak +2 from turn 1). The result includes `eraser_auto_activated: true/false` so the narrator can flag it. No-ops if Eraser isn't loaded, is already active, or is destroyed.
...
- speed_check_vs_black_ice: {type, character, target, interface_rank?} — Backend Speed Check on non-stealth Black ICE encounter. Pass: avoid effect (ICE still enters Initiative). Fail: take effect. Auto-skipped while target ICE is in a node under Backdoor Entry grace.
- backdoor_entry: {type, character, target (destination node), interface_rank?} — Project rulebook stealth-move action. 1 NA. Interface + Cloak boosters (Eraser +2 fires, HOMERULE) + d10 vs HIGHEST active ICE PER in destination + d10. Always enters node. Pass: emits `backdoor_entry_grace` op so ICE in destination skip encounter rolls this turn. Fail: ICE triggers normally — planner emits Speed Check / Patrol detection same turn.
