...
SCHEMA A - Route to Mechanics (default for in-character gameplay):
{
  ...
  "// clock advancement": "Backend-owned. Default 30s/turn (3s/round in combat). Use hud_state.time_override = {minutes, reason} to advance more. Never set hud_state.time or hud_state.date after the initial seed — those values are IGNORED once seeded.",
  "beats": ["<beat 1>", "<beat 2>", ...],
  ...
  "scene_state": {
    ...
    //   someone joins:  "_npcs_present_add": ["New contact"]
    //   someone leaves: "_npcs_present_remove": ["Mr. Johnson"]
...
**The clock is backend-owned.** Default advancement is 30 seconds per normal turn. You do **not** set the clock yourself.
...
- Travel, gearing up, matrix run: `minutes: 5–60`.
- Short rest, meal: `minutes: 60`.
- Sleep through to morning: `minutes: ~480` (8h).
...
Edge and condition monitors always come from runner_ops, never from hud_state.
        ...
        "properties": {
            ...
            "hud_state": {
                ...
                "properties": {
                    ...
                    "time_override": {
                        "type": "object",
                        "description": "THE ONLY way to advance the clock more than the default 30s/turn. Use for travel, extended actions, time skips. Omit for normal turns.",
                        "properties": {
                            "minutes": {"type": "number", "description": "Duration in minutes. For multi-hour or multi-day skips set this large enough to land the clock where the narrative ENDS (e.g. ~480 for sleep, 1440 for next day)."},
                            "reason": {"type": "string", "description": "Why this turn's duration differs (e.g. 'Legwork across the Sprawl', 'Sleep through to morning')"}
                        }
                    }
                }
            },
...
GAME_SYSTEM = {
    ...
    "combat_round_seconds": 3,  # Combat turns are 3 seconds in backend clock math
}
